  ...
  if (!course || !course.modules || course.modules.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-10 text-center border border-slate-200">
        <h3 className="text-lg font-bold text-slate-800">Course content unavailable</h3>
        <p className="text-xs text-slate-500 mt-1">This course does not have active modules yet.</p>
        <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-soft">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            <span className="text-xs font-bold text-slate-800">{progressPct}% Complete</span>
            <div className="w-28 sm:w-36 h-2 rounded-full bg-slate-100 mt-1 overflow-hidden">
              <div
          ...
          {course.certificate && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 text-amber-800 hover:bg-amber-100 rounded-xl text-xs font-bold transition shadow-xs"
            >
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-soft p-5">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl transition flex items-start gap-3 border ${
                      isCurrent
                        ? 'bg-brand-50/80 border-brand-300 text-slate-900 shadow-xs'
            ...
            {quiz && (
              <div className="mt-6 pt-5 border-t border-slate-100">
                <div className="p-4 rounded-xl bg-gradient-to-br from-brand-50 to-indigo-50 border border-brand-200">
                  ...
                  <button
                    ...
                    className="w-full py-2 px-3 bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold rounded-lg transition shadow-xs flex items-center justify-center gap-1.5"
                  >
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-soft p-6 sm:p-8">
            ...
            {activeModule.key_points && activeModule.key_points.length > 0 && (
              <div className="my-6 p-4 rounded-xl bg-amber-50/60 border border-amber-200/80">
            ...
            {/* Rich Content View */}
            <div className="prose max-w-none text-slate-800 text-sm leading-relaxed space-y-4 my-6 font-sans">
              <div className="whitespace-pre-wrap font-sans">{activeModule.content}</div>
            </div>

            {/* External / Simple Material Link if present */}
            {activeModule.material_url && (
              <div className="mt-8 p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 rounded-lg bg-sky-100 text-sky-700">
                    <BookOpen className="w-4 h-4" />
                  </div>
                  <div>
                    <h5 className="text-xs font-bold text-slate-800">Supplementary Material</h5>
                    <p className="text-[11px] text-slate-500">Official references and exercise documentation</p>
                  </div>
                </div>
                <a
                  ...
                  className="inline-flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold transition shadow-xs"
                >
            ...
            <div className="mt-10 pt-6 border-t border-slate-100 flex items-center justify-between">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 transition disabled:opacity-30 disabled:pointer-events-none"
              >
              ...
              {activeModuleIndex < totalModules - 1 ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-slate-900 hover:bg-slate-800 transition shadow-xs"
                >
                ...
                <button
                  onClick={() => onNavigate('quiz-player', { courseId: course.id, quizId: quiz.id })}
                  className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-brand-600 hover:bg-brand-700 transition shadow-sm shadow-brand-500/20"
                >
