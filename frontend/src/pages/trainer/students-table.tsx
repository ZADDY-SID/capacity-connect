  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="bg-white rounded-3xl p-8 border border-slate-200/80 shadow-soft">
        ...
        <div className="mt-6 p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-800">Automated "Needs Attention" Rule:</span>
            <span className="px-2 py-0.5 rounded bg-amber-100 text-amber-900 font-mono font-semibold">
              Progress &lt; 40% OR Quiz Score &lt; 50%
            </span>
          </div>
          <div className="text-slate-500">
            Total requiring support:{' '}
            <strong className="text-amber-700 font-bold">{needsAttentionCount} student(s)</strong>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-brand-500 focus:bg-white"
            />
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-soft overflow-hidden">
        {loading ? (
                ...
                {filteredStudents.map((st) => (
                  <tr
                    ...
                    className={`transition ${
                      st.needs_attention ? 'bg-amber-50/40 hover:bg-amber-50/70' : 'hover:bg-slate-50/80'
                    }`}
                  >
                    ...
                    <td className="py-4 px-6 text-slate-700 font-semibold">{st.course_title}</td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-slate-800">{st.progress}%</span>
                      </div>
                    </td>
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-slate-500">No students match your filter criteria.</p>
          </div>
        )}
      </div>
    </div>
