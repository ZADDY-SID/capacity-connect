    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200">
        ...
        >
          ...
          <span className="font-bold text-slate-800 text-sm">CAPACITY CONNECT</span>
        </div>
        <button
          onClick={() => setSidebarOpen(!sidebarOpen)}
          className="p-2 text-slate-600 rounded-lg"
        >
      ...
      {/* Sidebar Component */}
      <aside
        className={`fixed md:sticky top-0 z-50 md:z-0 h-screen w-64 bg-white border-r border-slate-200 flex flex-col justify-between transition-transform duration-200 ease-in-out ${
        ...
        <div className="p-3 border-t border-slate-100 bg-slate-50/50 space-y-3">
          ...
          <div className="flex items-center justify-between px-2 py-1.5">
            <div className="flex items-center gap-2.5 overflow-hidden">
              <div className="w-8 h-8 rounded-full bg-slate-200 flex-shrink-0 flex items-center justify-center font-bold text-slate-600 text-xs uppercase overflow-hidden border border-slate-300">
                {user?.avatar_url ? (
                  <img
                    src={user.avatar_url}
                    alt={user.name}
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        {/* Top bar */}
        <header className="bg-white border-b border-slate-200/80 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sticky top-0 z-30 shadow-xs">
          <div>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => onNavigate('landing')}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition"
            >
            ...
            <span className="text-xs font-medium text-slate-500">
              Role:{' '}
              <strong className="text-brand-600 uppercase tracking-wide">
                {user?.role}
              </strong>
            </span>
          </div>
